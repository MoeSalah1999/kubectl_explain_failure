...
FIXTURE_DIR = os.path.join(BASE_DIR, "node_pressure_eviction_cascade")
...
def test_node_pressure_eviction_cascade_golden():
    ...
    assert result["resolution"]["winner"] == "NodePressureEvictionCascade"
    assert "Evicted" in result["resolution"]["suppressed"]
    assert "NodeMemoryPressure" in result["resolution"]["suppressed"]
