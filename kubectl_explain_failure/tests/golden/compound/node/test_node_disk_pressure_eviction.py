...
FIXTURE_DIR = os.path.join(BASE_DIR, "node_disk_pressure_eviction")
...
def test_node_disk_pressure_then_eviction_golden():
    ...
    # Make object-required eviction rules eligible so precedence is exercised.
    pod_name = pod.get("metadata", {}).get("name", "pod1")
    context.setdefault("objects", {}).setdefault("pod", {})[pod_name] = pod
    ...
    assert result["resolution"]["winner"] == "NodeDiskPressureThenEviction"
    assert "Evicted" in result["resolution"]["suppressed"]
    assert "EphemeralStorageExceeded" in result["resolution"]["suppressed"]
    assert "NodePressureEvictionCascade" in result["resolution"]["suppressed"]
