...
FIXTURE_DIR = os.path.join(BASE_DIR, "ingress_controller_unavailable")
...
def test_ingress_controller_unavailable_golden():
    ...
    context["pvc"] = {
        "metadata": {"name": "portal-cache"},
        "status": {"phase": "Bound"},
    }
    context["serviceaccount"] = {"metadata": {"name": "portal-api"}}
    ...
    assert "IngressControllerUnavailable" in matched_rules
