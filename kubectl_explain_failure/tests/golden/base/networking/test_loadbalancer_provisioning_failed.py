...
FIXTURE_DIR = os.path.join(BASE_DIR, "loadbalancer_provisioning_failed")
...
def test_loadbalancer_provisioning_failed_golden():
    ...
    context["pvc"] = {
        "metadata": {"name": "checkout-cache"},
        "status": {"phase": "Bound"},
    }
    context["serviceaccount"] = {"metadata": {"name": "checkout-api"}}
    context["secret"] = {"metadata": {"name": "ecr-pull-secret"}}
    ...
    assert "LoadBalancerProvisioningFailed" in matched_rules
