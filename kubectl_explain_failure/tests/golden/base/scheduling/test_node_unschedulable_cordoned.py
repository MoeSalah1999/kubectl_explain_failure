...
def test_node_unschedulable_cordoned_golden():
    ...
    # Attach nodes for the specific rule
    context["objects"]["node"] = data.get("nodes")

    # Attach the pod object under "objects" as well
    context["objects"]["pod"] = {pod["metadata"]["name"]: pod}

    # Let normalize_context handle the timeline
