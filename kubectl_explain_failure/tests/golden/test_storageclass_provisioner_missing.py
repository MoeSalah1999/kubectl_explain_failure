...
FIXTURE_DIR = os.path.join(BASE_DIR, "storageclass_provisioner_missing")
...
def test_storageclass_provisioner_missing_golden():
    ...
    # Load objects into context
    context["objects"] = {}
    context["objects"]["pvc"] = data.get("pvc", {})
    context["objects"]["storageclass"] = data.get("storageclass", {})

    # Provide legacy blocking_pvc for engine PVC logic
    unbound_pvc_name, unbound_pvc_obj = next(iter(data["pvc"].items()))
    context["blocking_pvc"] = unbound_pvc_obj
