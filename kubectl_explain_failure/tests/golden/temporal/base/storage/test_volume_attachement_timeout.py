...
FIXTURE_DIR = os.path.join(BASE_DIR, "volume_attachement_timeout")
...
def test_volume_attachement_timeout_golden():
    ...
    for pvc in data.get("pvcs", []):
        context.setdefault("objects", {}).setdefault("pvc", {})[
            pvc.get("metadata", {}).get("name", "pvc1")
        ] = pvc
