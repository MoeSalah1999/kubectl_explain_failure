...
class PendingUnschedulableRule(FailureRule):
    ...
    def matches(self, pod, events, context) -> bool:
        ...
        if context.get("suppressed_rules"):
            ...
            for r in ["FailedScheduling", "PVCBoundNodeDiskPressureMount", "NodeDiskPressure"]:
        ...
        insufficient_resources = context.get("node_conditions", {}).get("MemoryPressure", False)

        # Match if pod cannot schedule for multiple reasons (compound behavior)
        if blocking_pvc is None and (failed_scheduling or node_pressure or insufficient_resources):
            return True
