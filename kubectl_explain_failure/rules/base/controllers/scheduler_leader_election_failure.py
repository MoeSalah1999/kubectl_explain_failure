...
class SchedulerLeaderElectionFailureRule(FailureRule):
    """
    Detects a kube-scheduler instance that cannot maintain its leader-election
    lease and is becoming unhealthy.

    Real-world behavior:
    - transient "failed to acquire lease" messages from a healthy standby
      scheduler are normal in HA control planes and should not diagnose a fault
    - repeated lease-renewal or leadership-loss events on the affected
      kube-scheduler pod, combined with readiness loss or restarts, are
      actionable
    - if kube-apiserver itself is down, APIServerUnreachable should dominate
      this more localized scheduler-path diagnosis
    """

    name = "SchedulerLeaderElectionFailure"
    category = "Controller"
    priority = 60
    deterministic = True
    ...
    CACHE_KEY = "_scheduler_leader_election_failure_candidate"
    ...
    def _is_scheduler_pod(self, pod_obj: dict[str, Any]) -> bool:
        if pod_obj.get("metadata", {}).get("namespace") != "kube-system":
            return False
        text = self._pod_text(pod_obj)
        return any(marker in text for marker in self.SCHEDULER_NAMES)

    def _scheduler_status(self, pod: dict[str, Any]) -> dict[str, Any] | None:
        statuses = pod.get("status", {}).get("containerStatuses", []) or []
        for status in statuses:
            if not isinstance(status, dict):
                continue
            if "scheduler" in str(status.get("name", "")).lower():
                return status
        return None
    ...
    def _candidate(
        ...
        if not self._is_scheduler_pod(pod):
            return None
        ...
        status = self._scheduler_status(pod)
        if status is None:
            return None
        ...
        return {
            ...
            "container_name": status.get("name", "kube-scheduler"),
    ...
    def explain(self, pod, events, context):
        ...
        if candidate is None:
            raise ValueError(
                "SchedulerLeaderElectionFailure explain() called without match"
            )
        ...
        chain = CausalChain(
            causes=[
                Cause(
                    code="SCHEDULER_REQUIRES_LEASE_RENEWAL",
                    message="kube-scheduler must keep renewing its coordination lease to remain the active scheduling leader",
                    role="scheduling_context",
                ),
                Cause(
                    code="SCHEDULER_LEADER_ELECTION_FAILURE",
                    message="kube-scheduler cannot renew or hold its leader-election lease",
                    role="scheduling_root",
                    blocking=True,
                ),
                Cause(
                    code="SCHEDULER_POD_UNHEALTHY",
                    message="The scheduler pod becomes unhealthy or restarts after losing leadership",
        ...
        evidence = [
            f"Recent leader-election failures target kube-scheduler pod '{pod_name}' with container '{container_name}' state={candidate['state_name']} restartCount={candidate['restart_count']}",
        ...
        if candidate["api_service_ready"] is True:
            evidence.append(
                "Kubernetes service 'kubernetes' still has ready API endpoints, which localizes the failure to this scheduler instance or its path to the API"
            )
        ...
        if candidate["api_service_ready"] is True:
            object_evidence["service:kubernetes"] = [
                "The kubernetes Service still has ready endpoints during the scheduler lease-renewal failure"
            ]
        ...
        return {
            "root_cause": "kube-scheduler cannot maintain its leader-election lease, so the scheduler pod becomes unhealthy",
            ...
            "likely_causes": [
                "Intermittent connectivity, packet loss, or timeouts between kube-scheduler and the Kubernetes API endpoint are breaking lease renewals",
                "Clock skew or long host stalls are causing scheduler lease deadlines to be missed",
                "Coordination Lease updates are failing because of RBAC, coordination API, or etcd-side errors",
                "The kube-scheduler process is unstable and restarts after losing leadership",
            ],
            "suggested_checks": [
                ...
                "Verify control-plane clock sync and the scheduler path to the Kubernetes API VIP",
            ],
        }
