...
class MutatingWebhookTimeoutRule(FailureRule):
    """
    Detects Pod creation failures caused by mutating admission webhook timeouts.

    Signals:
    - Event reason in {"FailedCreate", "Failed", "FailedAdmission"}
    - Event message contains "mutating webhook" and timeout indicators

    Interpretation:
    The API server attempted to call a mutating webhook, but the webhook
    ...
    def explain(self, pod, events, context):
        ...
        chain = CausalChain(
            causes=[
                ...
                Cause(
                    code="MUTATING_WEBHOOK_TIMEOUT",
                    message="Mutating webhook did not respond before admission timeout",
                    role="admission_root",
                    blocking=True,
                ),
                Cause(
                    code="POD_ADMISSION_BLOCKED",
                    message="Pod creation blocked due to mutating webhook timeout",
        ...
        return {
            "rule": self.name,
            "root_cause": "Mutating admission webhook timed out during pod creation",
            "confidence": 0.93,
            "blocking": True,
            "causes": chain,
            "evidence": [
                "Admission event indicates mutating webhook timeout",
                f"Pod: {pod_name}",
                f"Namespace: {namespace}",
            ],
            "object_evidence": {
                f"pod:{pod_name}": ["Mutating webhook timed out during admission"]
            },
            "likely_causes": [
